...
\doc{cb_pari_display_hist}{long (*cb_pari_display_hist)(long n)}
initialized to \kbd{NULL}. If set, called by \kbd{gp\_embedded}
in place of \kbd{gp\_display\_hist}.
...
\doc{cb_pari_long_help}{void (*cb_pari_long_help)(const char *s, long n)}
It is called in place of the external help, to handle \kbd{??}.
...
\fun{void}{gp_display_hist}{long n} print the history entry $n$ using the
usual GP format "$\%n = \dots$".

\fun{void}{str_display_hist}{pari_str *S, long n} as \kbd{gp\_display\_hist},
...
\fun{GEN}{cgetr}{long prec} allocates a \typ{REAL} of precision \kbd{prec}.
Sets $z[0]$ only.

\fun{GEN}{cgetc}{long prec} allocates a \typ{COMPLEX} whose real and
imaginary parts are \typ{REAL}s of precision \kbd{prec}.
...
\fun{long}{prec2ndec}{long x} converts a number of bits to a
...
\fun{long}{prec2lg}{long x} return the length of a \typ{REAL} of precision
\kbd{x}.

\fun{long}{lg2prec}{long x} return the maximal precision of a \typ{REAL}
of length \kbd{x}.

\fun{long}{nbits2prec}{long x} convert $x$ to a valid precision.

\fun{long}{nbits2extraprec}{long x} convert $x$ to a valid precision.
...
\fun{long}{prec2nbits}{long x} return $x$. For backward compatibility.
...
\fun{long}{realprec}{GEN x} precision of a \typ{REAL} in bits.

\fun{long}{bit_prec}{GEN x} precision of a \typ{REAL} in bits.

\fun{long}{precdbl}{long x} return $2*x$.
...
\fun{long}{valser}{GEN x} returns the $X$-adic valuation (for a \typ{SER}).
...
precision of~\kbd{x}, namely

\item if \kbd{x} is not zero: the length of \kbd{x} in \B-bit words;

\item if \kbd{x} is numerically equal to $0$, of exponent $e$:
the absolute accuracy \kbd{nbits2prec}$(e)$ if $e < 0$ and
\kbd{LOWDEFAULTPREC} if $e \geq 0$.

If \kbd{x} is of type \typ{COMPLEX}, returns the minimum of the precisions of
...
\kbd{prec} parameter in transcendental functions.
...
\fun{long}{evalvalp}{long x} convert $p$-adic valuation~\kbd{x} to
bitmask (second codeword of \typ{PADIC}). Raise overflow error if
...
\fun{long}{evalvalser}{long x} convert $X$-adic valuation~\kbd{x} to
bitmask (second codeword of \typ{SER}). Raise overflow error if
\kbd{x} is so large that the corresponding valuation cannot be represented.

\fun{long}{_evalvalser}{long x} same as \kbd{evalvalser} \emph{without} the
overflow check.
...
\fun{void}{setvalp}{GEN x, long s} sets the $p$-adic valuation
of~\kbd{x} to~\kbd{s}, if \kbd{x} is a \typ{PADIC}.

\fun{void}{setvalser}{GEN x, long s} sets the $X$-adic valuation
of~\kbd{x} to~\kbd{s}, if \kbd{x} is a \typ{SER}, respectively.
...
After \kbd{pari\_init(size, maxprime)}, a prime table \kbd{pari\_PRIMES} is
...
choosing \kbd{maxprime}; it is $2^{20}$ by default under \kbd{gp} and there
...
In fact, \kbd{maxprime} is automatically replaced by
$$\max(\kbd{maxprime}, 65557)\enspace.$$

In particular, PARI guarantees that the first 6547 primes, up to and
including 65557, are present in the table, even if you set \kbd{maxprime} to
zero in the \kbd{pari\_init} call.

\noindent Some convenience functions:

\fun{ulong}{maxprime}{void} the largest prime computable using our prime table.

\fun{ulong}{maxprimeN}{void} the index $N$ of the largest prime computable using
the prime table. I.e., $p_N = \kbd{maxprime()}$.

\fun{GEN}{prodprimes}{void} a vector $T$ where \kbd{T[i]} is the product of the
...
\fun{ulong}{maxprimelim}{void} the argument used for the last
\kbd{initprimetable}
call. This is at least $65537$ and the largest prime
less than or equal to this number is \kbd{maxprime}$()$.

\fun{long}{PRIMES_search}{ulong x} assumes $x \leq \kbd{maxprimelim()}$. Return
...
\fun{void}{initprimetable}{ulong maxprime} computes the prime table
\kbd{pari\_PRIMES} (of all primes $p < \kbd{maxprime}$).
This caches data allowing fast \kbd{prodprimes()}.
...
\fun{GEN}{cgetalloc}{size_t l, long t}, same as \kbd{cgetg(l, t)}, except
...
\fun{void}{set_avma}{ulong av} reset \kbd{avma} to \kbd{av}. You may think
of this as a simple \kbd{avma = av} statement, but PARI developpers modify
this statement in special code branches to detect garbage collecting
issues (by invalidating the PARI stack below \kbd{av}).

\fun{ulong}{get_avma}{void} return \kbd{avma}. Useful for languages that
...
\fun{GEN}{gc_const}{pari_sp av, GEN x} assumes that $x$ is either not on the
stack (clone, universal constant such as \kbd{gen\_0}) or was defined
before \kbd{av}.

\fun{GEN}{gc_stoi}{pari_sp av, long x} reset \kbd{avma} to \kbd{av} and return
\kbd{stoi(x)}.

\fun{GEN}{gc_utoi}{pari_sp av, long x} reset \kbd{avma} to \kbd{av} and return
\kbd{utoi(x)}.

\fun{GEN}{gc_utoipos}{pari_sp av, long x} reset \kbd{avma} to \kbd{av} and return
\kbd{utoipos(x)}.

\fun{GEN}{gc_all}{pari_sp av, int n, ...}. Assumes that $1 \leq n \leq
10$; This is similar to \kbd{gerepileall}, expecting $n$ further \kbd{GEN*}
arguments: the stack is cleaned and the corresponding \kbd{GEN} are
copied to the stack starting from \kbd{av} (in this order: the first argument
comes first), and the first such \kbd{GEN} is returned. To be used in the
following scenario:
\bprog
  GEN f(..., GEN *py)
  {
...
as a side effect. Not that we can later use \kbd{cgiv}$(y)$ to recover the
...
\fun{void}{dbg_fill_stack}{void} marks the unused portion of the stack
(between its bottom and \kbd{avma}) with repeated magic values:
\kbd{0xBADC0FFEE0DDF00D} on 64-bit archs, and \kbd{0xDEADBEEF} on 32-bit.
...
The \kbd{valgrind} framework more thorough possibilities but requires
...
\+& \kbd{p} &  current real precision in bits, rounded up to a multiple of
                \kbd{BITS\_IN\_LONG} \cr
...
\+&\kbd{DV}& optional loop variable.\cr
...
\fun{ulong}{pgener_Zl}{ulong p} returns the smallest positive integer which is
...
\fun{ulong}{get_Fl_red}{ulong p} returns a pseudoinverse \var{pi} for $p$.
...
pseudoinverse \var{pi} is equal to the Euclidean quotient of $B^2 - qB + B-1$
...
and remainders (\kbd{divll} or even \kbd{/} and \kbd{\%}) when $0\leq x < B$
...
\kbd{get\_Fl\_red} for arguments where the standard algorithm is preferable.

\fun{ulong}{divll_pre}{ulong x, ulong p, ulong pi}
as divll, where \var{pi} is the pseudoinverse of $p$.

\fun{ulong}{remll_pre}{ulong u1, ulong u0, ulong p, ulong pi} returns
the Euclidean remainder of $u_1\*2^B+u_0$ modulo $p$, assuming \var{pi} is the
...
assuming \var{pi} is the pseudoinverse of $p$.
...
modulo $p$, assuming \var{pi} is the pseudoinverse of $p$.

\fun{ulong}{Fl_addmul_pre}{ulong a, ulong b, ulong c, ulong p,  ulong pi}
returns $a+b\*c$ modulo $p$, assuming \var{pi} is the pseudoinverse of $p$.

\fun{ulong}{Fl_addmulmul_pre}{ulong a,ulong b, ulong c,ulong d, ulong p, ulong pi}
returns $a\*b+c\*d$ modulo $p$, assuming \var{pi} is the pseudoinverse of $p$.

\fun{ulong}{Fl_powu_pre}{ulong x, ulong n, ulong p, ulong pi} returns
$x^n$ modulo $p$, assuming \var{pi} is the pseudoinverse of $p$, or $0$
in which case we either use ordinary divisions if $p < B^{1/2}$ is small
and call \kbd{get\_Fl\_red} ourselves otherwise.

\fun{GEN}{Fl_powers_pre}{ulong x, long n, ulong p, ulong pi} returns
the vector (\typ{VECSMALL}) $(x^0, \dots, x^n)$, assuming \var{pi} is
the pseudoinverse of $p$, or $0$ in which case we either use ordinary
divisions if $p < B^{1/2}$ is small and call \kbd{get\_Fl\_red} ourselves
otherwise.

\fun{ulong}{Fl_log_pre}{ulong a, ulong g, ulong ord, ulong p, ulong pi}
as \kbd{Fl\_log}, assuming \var{pi} is the pseudoinverse of $p$, or $0$ in
which case we either use ordinary divisions if $p < B^{1/2}$ is small and
call \kbd{get\_Fl\_red} ourselves otherwise.

\fun{ulong}{Fl_sqrt_pre}{ulong x, ulong p, ulong pi} returns a square root
of $x$ modulo $p$, see \kbd{Fl\_sqrt}. We assume \var{pi} is the
pseudoinverse of $p$, or $0$ in which case we either use ordinary divisions
if $p < B^{1/2}$ is small and call \kbd{get\_Fl\_red} ourselves otherwise.

\fun{ulong}{Fl_sqrtl_pre}{ulong x, ulong l, ulong p, ulong pi}
returns a $l$-the root of \kbd{x} modulo \kbd{p}, assuming
$p$ prime, $p \equiv 1 \pmod{l}$, and \kbd{x} to be a $l$-th power modulo
\kbd{p}. We assume \var{pi} is the pseudoinverse of $p$, or $0$ in which case
we either use ordinary divisions if $p < B^{1/2}$ is small and call
\kbd{get\_Fl\_red} ourselves otherwise.

\fun{ulong}{Fl_sqrtn_pre}{ulong x, ulong n, ulong p, ulong pi, ulong *zn}
See \kbd{Fl\_sqrtn}, assuming \var{pi} is the pseudoinverse of $p$, or $0$ in
which case we either use ordinary divisions if $p < B^{1/2}$ is small and
call \kbd{get\_Fl\_red} ourselves otherwise.

\fun{ulong}{Fl_2gener_pre}{ulong p, ulong pi} return a generator of
the $2$-Sylow subgroup of $\F_p^*$, to be used in \kbd{Fl\_sqrt\_pre\_i}.
We assume \var{pi} is the pseudoinverse of $p$, or $0$ in which case we
either use ordinary divisions if $p < B^{1/2}$ is small and call
\kbd{get\_Fl\_red} ourselves otherwise.

\fun{ulong}{Fl_2gener_pre_i}{ulong ns, ulong p, ulong pi}
as \kbd{Fl\_2gener\_pre} where \kbd{ns} is a non-square modulo $p$.

\fun{ulong}{Fl_sqrt_pre_i}{ulong x, ulong s2, ulong p, ulong pi}
as \kbd{Fl\_sqrt\_pre} where \kbd{s2} is the element returned by
\kbd{Fl\_2gener\_pre}. We assume \var{pi} is the pseudoinverse of $p$, or $0$
...
\fun{GEN}{real_0}{long prec} is a shorthand for \kbd{real\_0\_bit(-prec)}.
...
\fun{long}{ZXX_pvalrem}{GEN x, GEN p, GEN *r} as \kbd{ZX\_pvalrem}, for
a \kbd{ZXX} $x$ (a \typ{POL} with \kbd{ZX} coefficients).
This function is not stack-clean.

\fun{long}{ZXV_pvalrem}{GEN x, GEN p, GEN *r} as \kbd{ZV\_pvalrem}, for
a \kbd{ZXV} $x$ (a \typ{VEC} with \kbd{ZX} coefficients).
This function is not stack-clean.
...
\fun{GEN}{Fp_double}{GEN x, GEN m} return $2x$ modulo $m$. Assume that $0
\leq x < m$. Not memory-clean, but suitable for \kbd{gerepileupto}.
...
\fun{GEN}{Fp_2gener_i}{GEN ns, GEN p} as \kbd{Fp\_2gener}, where \kbd{ns} is a
non-square modulo $p$.
...
+ C$ modulo $N$. Return \kbd{NULL} if there are no solutions. Else returns
 $[M, v]$ where $M \mid N$ and the \kbd{FpV} $v$ of distinct integers
...
\fun{GEN}{coredisc2_fact}{GEN fa, long s, GEN *pP, GEN *pE} let $d$ be an
integer congruent to $0$ or $1$ mod $4$. Return
$D = \kbd{coredisc}(d)$ assuming that \kbd{fa} is the factorization of $|d|$
and $s d > 0$ ($s$ is the sign of $d$). Set \kbd{*pP} and \kbd{*pE} to the
factorization of the conductor $f$ such that $d = Df^2$, where $P$ is a
\typ{VEC} of primes and $E$ a \typ{VECSMALL} of exponents.

\fun{ulong}{coredisc2u_fact}{GEN fa, long s, GEN *pP, GEN *pE} let $d$ be an
integer congruent to $0$ or $1$ mod $4$ whose absolute value fits in an
\kbd{ulong}. Return the absolute value of $D = \kbd{corediscs}(d)$ assuming
that \kbd{fa} is the factorization of $|d|$ and $s d > 0$ ($s$ is the sign of
$d$ and $D$). Set \kbd{*pP} and \kbd{*pE} to the factorization of the
conductor $f$ such that $d = Df^2$, where $P$ is a \typ{VECSMALL} of primes
and $E$ a \typ{VECSMALL} of exponents.
...
\fun{long}{maxomegau}{ulong x} return the optimal $B$ such that $\omega(n)
\leq B $ for all $n \leq x$.

\fun{long}{maxomegaoddu}{ulong x} return the optimal $B$ such that $\omega(n)
...
\noindent Possible letters are:
  ...
  \kbd{V}: vector (\typ{VEC} or \typ{COL}), treated as a row vector
...
and \kbd{y} (assumed to have compatible dimensions), assuming \var{pi} is the
...
\kbd{Flm} \kbd{x} by \kbd{y} assuming \var{pi} is the pseudoinverse of $p$,
...
scalar product of \kbd{x} and \kbd{y} assuming \var{pi} is the pseudoinverse of
$p$, or $0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
\fun{GEN}{Flv_inv}{GEN x, ulong p} returns the vector of inverses of the
elements of $x$ (as a \kbd{Flv}). Use Montgomery's trick.
...
\kbd{y} (assumed to have compatible dimensions), assuming \var{pi} is the
pseudoinverse of $p$, or $0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.

\fun{GEN}{Flm_sqr}{GEN x, ulong p} squares \kbd{x}
...
\fun{int}{F2v_subset}{GEN x, GEN y} return $1$ if the set of indices of
...
\fun{GEN}{zero_F3m_copy}{long m, long n} creates a \kbd{F3m} with \kbd{m} x
\kbd{n} components set to $0$.
...
\fun{GEN}{FlxqV_dotproduct_pre}{GEN x, GEN y, GEN T, ulong p, ulong pi} where
\var{pi} is the pseudoinverse of $p$, or $0$ in which case we
...
\fun{GEN}{FpX_halfgcd_all}{GEN x, GEN y, GEN p, GEN *pt_a, GEN *pt_b}
as \kbd{FpX\_halfgcd}, in addition, if \kbd{pt\_a} (resp. \kbd{pt\_b}) is not
\kbd{NULL}, \kbd{*pt\_a} (resp. \kbd{*pt\_b})
is set to \kbd{a} (resp. \kbd{b}).
...
If \kbd{u} is \kbd{NULL}, \kbd{*u} is not computed which is a bit faster.
...
\fun{GEN}{FpXV_FpC_mul}{GEN V, GEN W, GEN p} multiplies a nonempty row
...
\fun{GEN}{FpXV_composedsum}{GEN V, GEN p}, \kbd{V} being a vector of \kbd{FpX},
returns their composed sum, see \kbd{FpX\_composedsum}.
...
\fun{GEN}{FpX_roots_mult}{GEN f, long n, GEN p}
returns the roots in \kbd{\Z/p\Z} with multiplicity at least $n$ of
the \kbd{FpX}~\kbd{f} (without multiplicity, as a vector of \kbd{Fp}s).
Assumes that \kbd{p} is prime.
...
\fun{GEN}{FpX_factcyclo}{ulong n, GEN p, ulong m} returns the factors
...
\fun{GEN}{random_Fq}{GEN T, GEN p} returns a random \kbd{Fq}
...
\fun{GEN}{FpXn_div}{GEN x, GEN y, long n, GEN p} return $x/y\pmod{X^n}$.
...
\fun{GEN}{FpXQXV_red}{GEN z, GEN T, GEN p}, \kbd{z} a \typ{VEC} of \kbd{ZXX}. Applies
\kbd{FpX\_red} componentwise and returns the result (and we obtain a vector
of \kbd{FpXQX}s).

\fun{GEN}{FpXQXT_red}{GEN z, GEN T, GEN p}, \kbd{z} a tree of \kbd{ZXX}. Applies
\kbd{FpX\_red} to each leaf and returns the result (and we obtain a tree
of \kbd{FpXQX}s).
...
\fun{GEN}{FpXQX_halfgcd_all}{GEN x, GEN y, GEN T, GEN p, GEN *ptu, GEN *ptv}
...
\fun{GEN}{FpXQXn_div}{GEN x, GEN y, long n, GEN T, GEN p}
...
\fun{GEN}{FpXQX_roots_mult}{GEN f, long n, GEN T, GEN p}
returns the roots in \kbd{\Z/p\Z} with multiplicity at least $n$ of
the \kbd{FpXQX}~\kbd{f} (without multiplicity, as a vector of \kbd{FpXQ}s).
...
\fun{GEN}{Flx_get_red_pre}{GEN T, ulong p, ulong pi} as \kbd{Flx\_get\_red}.
We assume \var{pi} is the pseudoinverse of $p$, or $0$ in which case we
assume \kbd{SMALL\_ULONG}$(p)$.
...
\fun{GEN}{Flx_mul_pre}{GEN x, GEN y, ulong p, ulong pi}

\fun{GEN}{Flx_Fl_mul}{GEN y, ulong x, ulong p}

\fun{GEN}{Flx_Fl_mul_pre}{GEN y, ulong x, ulong p, ulong pi}
...
\fun{GEN}{Flx_sqr_pre}{GEN x, ulong p, ulong pi}

\fun{GEN}{Flx_powu}{GEN x, ulong n, ulong p} return $x^n$.

\fun{GEN}{Flx_powu_pre}{GEN x, ulong n, ulong p, ulong pi}

\fun{GEN}{Flx_convol}{GEN x, GEN y, ulong p} return the-term by-term product of
$x$ and $y$.

\fun{GEN}{Flx_divrem}{GEN x, GEN y, ulong p, GEN *pr},
here $p$ must be prime.

\fun{GEN}{Flx_divrem_pre}{GEN x, GEN y, ulong p, ulong pi, GEN *pr}

\fun{GEN}{Flx_div}{GEN x, GEN y, ulong p},
here $p$ must be prime.

\fun{GEN}{Flx_div_pre}{GEN x, GEN y, ulong p, ulong pi}

\fun{GEN}{Flx_rem}{GEN x, GEN y, ulong p},
here $p$ must be prime.

\fun{GEN}{Flx_rem_pre}{GEN x, GEN y, ulong p, ulong pi}
...
\fun{GEN}{Flx_Frobenius_pre}{GEN T, ulong p, ulong pi}

\fun{GEN}{Flx_matFrobenius}{GEN T, ulong p}
here $p$ must be prime.

\fun{GEN}{Flx_matFrobenius_pre}{GEN T, ulong p, ulong pi}
...
\fun{GEN}{Flx_gcd_pre}{GEN a, GEN b, ulong p}
...
\fun{GEN}{Flx_halfgcd_all}{GEN x, GEN y, ulong p, GEN *pt_a, GEN *pt_b}

\fun{GEN}{Flx_halfgcd_all_pre}{GEN x, GEN y, ulong p, GEN *pt_a, GEN *pt_b}
...
\fun{ulong}{Flx_extresultant_pre}{GEN a, GEN b, ulong p, ulong pi, GEN *ptU, GEN *ptV}

\fun{GEN}{Flx_composedprod}{GEN P, GEN Q, ulong p} if
$P=a\prod_{i=1}^m(x-p_i)$ and $Q=b\prod_{j=1}^n(x-q_j)$ in some suitable
algebraic extension, return $a^n\*b^m\prod_{i,j}(x-p_i\*q_j)$.

\fun{GEN}{Flx_composedsum}{GEN P, GEN Q, ulong p} if
$P=a\prod_{i=1}^m(x-p_i)$ and $Q=b\prod_{j=1}^n(x-q_j)$ in some suitable
algebraic extension, return $a^n\*b^m\prod_{i,j}(x-(p_i+q_j))$.
...
\fun{GEN}{FlxV_composedsum}{GEN V, ulong p}, \kbd{V} being a vector of \kbd{Flx},
returns their composed sum, see \kbd{Flx\_composedsum}.
...
\fun{GEN}{Flxq_sqrt_pre}{GEN x, GEN T, ulong p, ulong pi}
...
\fun{GEN}{FlxqX_div_by_X_x}{GEN a, GEN x, GEN T, ulong p, GEN *r}

\fun{GEN}{FlxqX_div_by_X_x_pre}{GEN a, GEN x, GEN T, ulong p, ulong pi, GEN *r}
...
\fun{GEN}{FlxqX_halfgcd_all}{GEN x, GEN y, GEN T, ulong p, GEN *a, GEN *b},
see \kbd{FpX\_halfgcd\_all}.

\fun{GEN}{FlxqX_halfgcd_all_pre}{GEN x, GEN y, GEN T, ulong p, ulong pi, GEN *a, GEN *b},
see \kbd{FpX\_halfgcd\_all\_pre}.

\fun{GEN}{FlxqX_resultant}{GEN x, GEN y, GEN T, ulong p}, see \kbd{FpX\_resultant}.

\fun{GEN}{FlxqX_resultant_pre}{GEN x, GEN y, GEN T, ulong p, ulong pi}
...
\fun{GEN}{FlxqX_composedsum}{GEN P, GEN Q, GEN T, ulong p}

\fun{GEN}{FlxqX_disc}{GEN x, GEN T, ulong p}

\fun{GEN}{FlxqX_eval}{GEN x, GEN y, GEN T, ulong p} evaluates the
\kbd{FlxqX}~\kbd{x} at the \kbd{Flxq}~\kbd{y}. The result is an~\kbd{Flxq}.
...
\fun{GEN}{F2xqV_roots_to_pol}{GEN V, GEN T, long v} as
\kbd{FqV\_roots\_to\_pol} returning an \kbd{F2xqX} in variable $v$.
...
\fun{GEN}{F2xX_shift}{GEN a, long n, long sv}, as \kbd{RgX\_shift\_shallow},
where $v$ is the secondary variable.
...
\fun{GEN}{F2xqX_halfgcd_all}{GEN x, GEN y, GEN T, GEN *a, GEN *b}

\fun{GEN}{F2xqX_resultant}{GEN x, GEN y, GEN T}

\fun{GEN}{F2xqX_disc}{GEN x, GEN T}

\fun{GEN}{F2xqXV_prod}{GEN V, GEN T}
...
\fun{GEN}{absZ_factor_limit_strict}{GEN n, ulong lim, GEN *pU}. This function
is analogous to
...
but \kbd{lim} may be larger) and then the full \kbd{addprimes} prime table
(whatever their size).
...
\kbd{NULL} (no cofactor) or $[q,k]$, where $q > \kbd{lim}$ and $k > 0$.
The prime divisors of $q$ are greater than \kbd{lim} and not in the
\kbd{addprimes} table, $q$ is not a pure power, and $q^k$ is the largest
...
\fun{GEN}{mpprimorial}{long n} return $n\#$, the product of primes less than
...
\fun{long}{ZV_max_expi}{GEN x} returns the number of bits of the largest
entry in $x$.
...
\fun{GEN}{ZM2_sqr}{GEN x} returns $x^2$, where $x$ is a two-by-two \kbd{ZM}.
...
\fun{long}{ZM_max_expi}{GEN x} returns the number of bits of the largest
entry in $x$.
...
\fun{GEN}{RgM_div}{GEN a, GEN b} returns $ab^-1$ or \kbd{NULL} if this turns
...
\fun{GEN}{RgM_Cholesky}{GEN M} given a square symmetric \typ{MAT} $M$, return
$R$ such that $^t{}R\*R = M$, or \kbd{NULL} is no solution is found ($M$ is
...
$M$ being a \kbd{RgMs} with \kbd{nbrow} rows, $A$ being a list of row indices,
...
\fun{GEN}{ZX_unscale_divpow}{GEN P, GEN h, long k} given a \kbd{ZX} $P$,
a \typ{INT} $h$ and $k > 0$, returns $P(hx)/h^k$ assuming the result has
...
\fun{GEN}{ZX_squff}{GEN T, GEN *E} write the nonzero \kbd{ZX} $T$ as a
product $\prod T_i^{e_i}$ with the $e_1 < e_2 < \cdots$ all distinct and the
$T_i$ pairwise coprime. Return the vector of the $T_i$, and set \kbd{*E} to
the vector of the $e_i$, as a \typ{VECSMALL}. For efficiency, powers of $x$
should have been removed from $T$ using \kbd{ZX\_valrem}, but the result is
...
\fun{GEN}{ZX_composedsum}{GEN P, GEN Q} if
...
Assume that $x$ is a \kbd{QX} and $T$ is a \kbd{ZX}. Meant to precompute
...
\fun{GEN}{QXQX_homogenous_evalpow}{GEN P, GEN a, GEN B, GEN T}
...
$0$ to the degree of $P$ (\kbd{QXQ\_powers(b,degpol(P),T)}).
...
\fun{GEN}{RgX_homogenize}{GEN P, long v} Return the homogenous polynomial
...
\fun{GEN}{RgX_homogenous_evalpow}{GEN P, GEN a, GEN B}
Evaluate the homogenous polynomial associated to the univariate polynomial
$P$ on $(a,b)$ where $B$ is the vector of powers of $b$ with exponents
$0$ to the degree of $P$ (\kbd{gpowers(b,degpol(P))}).
...
\fun{GEN}{RgX_mul2n}{GEN y, long n} multiplies the \kbd{RgX} \kbd{y}
by $2^n$.
...
\fun{GEN}{RgXV_prod}{GEN V}, \kbd{V} being a vector of \kbd{RgX},
returns their product.
...
disposed of individually with a \kbd{cgiv(r)}.
...
returns a $2\times 2$ \typ{MAT} $M$ with \typ{POL} entries,
such that $M*[x,y]~==[a,b]~$ such that
$\deg a \geq \ceil{\max(\deg x,\deg y))/2} > \deg b$.

\fun{GEN}{RgX_halfgcd_all}{GEN x, GEN y, GEN *pt_a, GEN *pt_b}
as \kbd{RgX\_halfgcd}, in addition, if \kbd{pt\_a} (resp. \kbd{pt\_b}) is not
\kbd{NULL}, \kbd{*pt\_a} (resp. \kbd{*pt\_b})
is set to \kbd{a} (resp. \kbd{b}).
...
\fun{GEN}{RgXV_rescale}{GEN v, GEN h} apply \kbd{RgX\_unscale} to a vector
of \kbd{RgX}.
...
\fun{GEN}{RgX_affine}{GEN P, GEN a, GEN b} Return $P(aX + b)$
...
\fun{GEN}{RgXn_div}{GEN a, GEN b, long n} returns $a/b$ modulo $X^n$,
where $a$ and $b$ are \typ{POL}s in the variable $X$ and $n \geq 0$. Uses
Newton-Raphson/Karp-Markstein algorithm.

\fun{GEN}{RgXn_div_i}{GEN a, GEN b, long n} as \tet{RgXn_div} without final garbage
collection (suitable for \kbd{gerepileupto}).
...
\fun{GEN}{RgXQ_minpoly}{GEN x, GEN T, long v} returns the minimal
polynomial of \kbd{Mod(x, T)}, in variable $v$.
...
\fun{GEN}{RgXQX_div}{GEN x, GEN y, GEN T}

\fun{GEN}{RgXQX_rem}{GEN x, GEN y, GEN T}
...
\fun{GEN}{gprec_w}{GEN x, long prec} returns a shallow copy of $x$ whose
\typ{REAL} components have their precision changed to \kbd{prec} bits.
This is often more useful than \kbd{gprec}.

\fun{GEN}{gprec_wtrunc}{GEN x, long prec} returns a shallow copy of $x$ whose
\typ{REAL} components have their precision \emph{truncated} to \kbd{prec}
bits. Contrary to \kbd{gprec\_w}, this function may never increase
the precision of~$x$.

\fun{GEN}{gprec_wensure}{GEN x, long prec} returns a shallow copy of $x$ whose
\typ{REAL} components have their precision \emph{increased} to at least $prec$
bits. Contrary to \kbd{gprec\_w}, this function may never decrease
the precision of~$x$.
...
\fun{GEN}{rfrac_to_ser_i}{GEN F, long l} internal variant of
\kbd{rfrac\_to\_ser}, neither memory-clean nor suitable for
\kbd{gerepileupto}.
...
\fun{GEN}{normalizeser}{GEN x} applied to an unnormalized \typ{SER}~\kbd{x}
...
\fun{GEN}{uutoQ}{ulong x, ulong y} returns the \typ{INT} or \typ{FRAC} $x/y$;
no assumptions.
...
\fun{GEN}{quadpoly_i}{GEN D} creates the canonical quadratic polynomial of
discriminant $D$. Assume that the \typ{INT} $D$ is congruent to $0,1$ mod $4$
and not a square.
...
\fun{GEN}{retmkmat22}{GEN a, GEN b, GEN c, GEN d} return the $2$ by $2$
\typ{MAT} with successive rows $[a,b]$ and $[c,d]$.
...
The parameter $e$ can be set to \kbd{NULL} if an error estimate is not
...
\fun{long}{vecindexmin}{GEN x} returns the index for a minimal element of $x$
...
\fun{GEN}{ZM_merge_factor}{GEN fx, GEN fy}
as \kbd{merge\_factor}, where \kbd{fx} and \kbd{fy} are factorization matrices of
integers.
...
\fun{GEN}{gdivgu}{GEN x, ulong u} return $x/u)$.

\fun{GEN}{gdivgunextu}{GEN x, ulong u} return $x/(u(u+1))$. If $u(u+1)$
does not fit into an \kbd{ulong}, it is created and left on the stack for efficiency.

\fun{GEN}{divrunextu}{GEN x, ulong i} as \kbd{gdivgunextu} for a
\typ{REAL}~$x$.
...
\funno{GEN}{g\op gu}{GEN x, ulong u}

\funno{GEN}{g\op sg}{long s, GEN y}

\funno{GEN}{g\op ug}{ulong u, GEN y}
...
\fun{GEN}{gmulsg}{long s, GEN x}, \fun{GEN}{gmulgu}{GEN x, ulong u},
\fun{GEN}{gmulug}{GEN x, ulong u},
...
\fun{GEN}{gdivsg}{long s, GEN x}, \fun{GEN}{gdivgu}{GEN x, ulong u},
...
\fun{GEN}{RgXY_cxevalx}{GEN T, GEN x, GEN xi} Apply \kbd{RgX\_cxeval} to
...
\fun{GEN}{qf_RgM_apply}{GEN q, GEN M} $q$ is a symmetric $n\times n$ matrix,
$M$ an $n\times k$ matrix, return $M' q M$.

\fun{GEN}{qf_ZM_apply}{GEN q, GEN M} as above assuming that both
...
\fun{GEN}{modRr_i}{GEN x, GEN y, GEN iy} let $x$ be a \typ{INT}, a \typ{REAL} or
\typ{FRAC} and let $y$ be a \typ{REAL} and $\var{iy} = \kbd{invr}(y)$. Return
...
\fun{int}{cx_approx0}{GEN a, GEN b} test whether (\typ{INT}, \typ{FRAC},
\typ{REAL}, or \typ{COMPLEX} of those) $a$ is approximately $0$, where $b$
is a reference point. A non-$0$ \typ{REAL} component $x$ is
approximately $0$ if
...
\fun{GEN}{qfb_ZM_apply}{GEN q, GEN g} returns $q \circ g$.
...
\fun{GEN}{FFX_halfgcd_all}{GEN x, GEN y, GEN a, GEN *ptA, GEN *ptB}
as \kbd{FFX\_halfgcd}, in addition, if \kbd{ptA} (resp. \kbd{ptB}) is not
\kbd{NULL}, \kbd{*ptA} (resp. \kbd{*ptB}) is set to \kbd{A} (resp. \kbd{B}).
...
\fun{GEN}{FFV_roots_to_pol}{GEN V, GEN ff, long v}
\kbd{V} being a vector over the finite field given by \kbd{ff} (\typ{FFELT}),
returns the monic \typ{POL} $\prod_i (\kbd{pol\_x[v]} - \kbd{V[i]})$.
...
for \kbd{gerepileupto}. The complexity is $O(b / \log(v/u))$, where $b$ is
\kbd{prec2nbits(prec)}, so becomes impractical when $v/u$ is too close to
$1$.
...
$b$ are real numbers (of arbitrary type, although \typ{INT} is treated
...
\fun{GEN}{trans_evalgen}{const char *fun, void *E, GEN (*f)(void*,GEN,long), GEN x, long prec}, general variant evaluating $f(E, x, \var{prec})$, where the
...
The argument $x$ is assumed to be a \typ{PADIC}.

\fun{GEN}{Qp_exp}{GEN x} shortcut for \kbd{gexp(x, /*ignored*/prec)}

\fun{long}{Qp_exp_prec}{GEN x} number of terms to sum in the $\exp(x)$
...
\fun{GEN}{Qp_zeta}{GEN x} shortcut for \kbd{gzeta(x, /*ignored*/prec)};
assume that $x\neq 1$.

\fun{GEN}{Qp_zetahurwitz}{GEN x, GEN y, long k} shortcut for
 \kbd{zetahurwitz(x, y, k, /*ignored*/prec)}.

\fun{GEN}{Qp_psi}{GEN x} shortcut fo \kbd{gpsi(x, /*ignored*/prec)}.
...
\fun{GEN}{groupelts_solvablesubgroups}{GEN elts} where \var{elts} is the list of
...
\fun{GEN}{pari_base64}{const char *s} convert the string to base64 (RFC4648
"+/" with padding).
...
messages if the debug level for \kbd{"files"} is high enough.
...
\fun{int}{pari_err_display}{GEN err} displays the error message corresponding
to \kbd{err}. Always return $0$. Default value of the callback
\tet{cb_pari_err_handle}.
...
\fun{GEN}{hash_keys_GEN}{hashtable *h} return in a \typ{VEC} the keys
stored in hashtable $h$ (which are assumed to be \kbd{GEN}s).
...
\fun{GEN}{RgV_F2v_extract_shallow}{GEN V, GEN x} extract components of
...
The object returned has the same type as $A$ (\typ{VECSMALL}, \typ{VEC},
\typ{COL} or \typ{MAT}). Assume $j_1 \leq j_2$ or $j_2 = j_1 - 1$ (return
empty vector/matrix).

\fun{GEN}{vecsplice}{GEN A, long j} return $A$ with $j$-th entry removed
(\typ{VEC}, \typ{COL}) or $j$-th column removed (\typ{MAT}).

\fun{GEN}{veclast}{GEN A} return the last entry of $A$
(\typ{VEC}, \typ{COL}) or last column of $A$ (\typ{MAT}). Shallow, undefined
if \kbd{lg(A)} is $1$.
...
\fun{GEN}{vecsqr}{GEN a}
returns the vector whose $i$-th component is \kbd{gsqr}$(a[i])$.
...
The following sorting functions assume all entries of the \typ{VECSMALL}
$v$ satisfy $0 \leq v[i] \leq M$ and use a counting sort, in linear time
$O(\#v + M)$:

\fun{void}{vecsmall_counting_sort}{GEN v, long M} sorts the \typ{VECSMALL}
\kbd{v} in place.

\fun{GEN}{vecsmall_counting_indexsort}{GEN v, long M} as
\kbd{vecsmall\_indexsort} using a counting sort.

\fun{GEN}{vecsmall_counting_uniq}{GEN v, long M} as
\kbd{vecsmall\_uniq} using a counting sort.
...
\fun{GEN}{closure_callgen0}{GEN C, long prec} evaluate the \typ{CLOSURE}
...
\fun{GEN}{sd_factorlimit}{const char *v, long flag}
