# 7.4: DES
    ...
    • The algorithm starts with a plaintext 'm' of 64 bits and consists of three main stages:
        ...
        STAGE 1 (PREPROCESSING): DERIVE L₀R₀ and the 56-bit key 
            ...
            (1) the bits of 'm' are permuted by a fixed initial permutation (IP) to obtain m₀ = IP(m)
                ...
                - this means that the 58th bit of m becomes the first bit of m₀, the 50th bit of m becomes the second bit of m₀, etc

            - write m₀ as L₀R₀, where L₀ = first 32 bits, R₀ = last 32 bits

            (2) remove the parity bits (bits 8, 16, 24, 32, 40, 48, 56, 64) from the 64-bit key 'K' and permute the bits based on the key permutation table:
                ...
                * IMPORTANT CLARIFICATION: C₀D₀ and L₀R₀ are created ONCE in this stage because they serve as the "seed" for all the rounds that follow in stage 2

        STAGE 2: THE KEY SCHEDULE AND DES FUNCTION 
        ...
        STAGE 3 (POST PROCESSING): 
            ...
            - lets recap stage 2 so we can understand what is happening in here (reference figure 7.4 as well):
                
            - we ran a loop 16 times; in every round:
            ...
            - in every round of Stage 2, the halves RᵢLᵢ essentially "crossed over"; for some mathematical reasons (to make decryption work the same as encryption), DES requires one final "undo" of that crossover

            (1) swap the L₁₆ and R₁₆ from stage 2 to obtain R₁₆L₁₆ (2 32-bit strings)
            
            (2) apply the inverse of the initial permutation to get the ciphertext c = IP⁻¹(R₁₆L₁₆)

                - recall the initial permutation table is as follows:
                ...
                - previously this meant that the 58th bit of the original plaintext (m) became the 1st bit of the permuted plaintext (m₀), and the 50th bit of m became the 40th bit of m₀, etc

                - now what we are doing is the inverse of this, so the bit in position 40 of the input block R₁₆L₁₆ becomes the first bit of the ciphertext 'c', the bit in position 8 becomes the 2nd bit of the ciphertext, etc

                - we can make the inverse permution table (IP⁻¹) below:
                ...
                * the position in the grid is the destination; the number in the grid is the source